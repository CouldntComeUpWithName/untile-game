...
int App(std::string_view title)
{
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "untile", nullptr, nullptr);
    if (window == nullptr)
        return 1;
...
std::uptr<utd::window> utd::window::create(utd::u32 width, utd::u32 height, const std::string& title)
{
    return std::make_unique<utd::glfw_window>(width, height, title);
}
